...
```{r,echo=FALSE}
knitr::opts_chunk$set(echo = FALSE, fig.path = "images/formal-", comment = NA)
```
...
```{r, fig.cap="Hypothetical Advertising Campaign"}
set.seed(1)
par(mar = c(5, 4, 2, 2) + .1)
x <- c(rep(0, 7), rep(1, 7), rep(0, 7))
tt <- 1:21
e <- rnorm(21, 0, 10)
y <- 200 + 100 * x + e
plot(tt, y, xlab = "Day", ylab = "Total Daily Sales ($)", type = "b", pch = 20)
rug(tt[x == 1])
```
...
```{r}
data.frame(sales = y, campaign = x, day = tt)
```

Given this data and the primary model above, we'd estimate $\beta$ to be $`r round(coef(fit)[2], 2)`, which is not far off from our original guess of $100.
...
```{r, fig.cap="More Realistic Daily Sales Data"}
set.seed(23)
par(mar = c(5, 4, 2, 2) + .1)
x <- c(rep(0, 7), rep(1, 7), rep(0, 7))
tt <- 1:21
e <- rnorm(21, 0, 30)
y <- 100 - 20 * cos(2 * pi * tt / 21) + 20 * x + e
plot(tt, y, xlab = "Day", ylab = "Total Daily Sales ($)", type = "b", pch = 20)
rug(tt[x == 1])
```
...
Let's take our primary model, which just includes the outcome and the indicator of our ad campaign as a key predictor. Using that model we estimate $\beta$, the increase in daily sales due to the ad campaign, to be $`r round(coef(fit)[2], 2)`.

```{r}
fit2 <- lm(y ~ x + poly(tt, 2))
```
...
where $t$ now indicates the day number (i.e. $1,2,\dots,21$). What we have done is add a quadratic function of $t$ to the model to allow for some curvature in the trend (as opposed to a linear function that would only allow for a strictly increasing or decreasing pattern). Using this model we estimate $\beta$ to be $`r round(coef(fit2)[2], 2)`, which is somewhat less than what the primary model estimated for $\beta$.

```{r}
fit3 <- lm(y ~ x + poly(tt, 4))
```

We can fit one final model, which allows for an even more flexible background trend--we use a 4th order polynomial to represent that trend. Although we might find our quadratic model to be sufficiently complex, the purpose of this last model is to just push the envelope a little bit to see how things change in more extreme circumstances. This model gives us an estimate of $\beta$ of $`r round(coef(fit3)[2], 2)`, which is in fact larger than the estimate from our primary model.

```{r}
b1 <- round(coef(fit)[2], 2)
b2 <- round(coef(fit2)[2], 2)
b3 <- round(coef(fit3)[2], 2)
```
...
| ------| ---- | --------------------:|
| Model 1 (primary) | No confounders | $`r b1` |
| Model 2 (secondary) | Quadratic time trend | $`r b2` |
| Model 3 (secondary) | 4th order time trend | $`r b3` |

#### Evaluation

Determining where to go from here may depend on factors outside of the dataset. Some typical considerations are

1. **Effect size**. The three models present a range of estimates from $`r min(c(b1, b2, b3))` to $`r max(c(b1, b2, b3))`. Is this a large range? It's possible that for your organization a range of this magnitude is not large enough to really make a difference and so all of the models might be considered equivalent. Or you might consider these 3 estimates to be significantly different from each other, in which case you might put more weight on one model over another. Another factor might be the cost of the advertising campaign, in which case you would be interested in the return on your investment in the ads. An increase in $`r b2` per day might be worth it if the total ad cost were $10 per day, but maybe not if the cost were $20 per day. Then, you might need the increase in sales to be higher to make the campaign worthwhile. The point here is that there's some evidence from your formal model that the ad campaign might only increase your total daily sales by `r b2`, however, other evidence says it might be higher. The question is whether you think it is worth the risk to buy more ads, given the range of possibilities, or whether you think that even at the higher end, it's probably not worth it.
...
```{r,fig.cap="Ideal Classification Scenario"}
set.seed(2)
x <- rnorm(100)
logit.p <- 0 + 8 * x
y <- rbinom(100, 1, plogis(logit.p))
par(mar = c(5, 4, 2, 2) + .1)
plot(x, y, xlab = "Predictor", ylab = "Binary outcome", yaxt = "n", 
     ylim = c(-0.25, 1.25))
axis(2, 0:1, as.character(0:1))
first <- sort(x)[which(y[order(x)] == 1)[1]] - 0.05
last <- sort(x)[tail(which(y[order(x)] == 0), 1)] + 0.05
polygon(c(first, last, last, first), c(-1, -1, 2, 2), border = NA, 
        col = rgb(0, 0, 0, 0.3))
```
...
data(GermanCredit)
set.seed(20)
train.idx <- createDataPartition(y = GermanCredit$Class, 
                                 p = 0.75, 
                                 list = FALSE)
training <- GermanCredit[train.idx, ]
testing <- GermanCredit[-train.idx, ]
```

For this example we will use data on the credit worthiness of individuals. The dataset is taken from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/datasets/Statlog+(German+Credit+Data)). The dataset classifies individuals into "Good" or "Bad" credit risks and includes a variety of predictors that may predict credit worthiness. There are a total of `r nrow(GermanCredit)` observations in the dataset and `r ncol(GermanCredit)` features. For the purpose of this exposition, we omit the code for this example, but the code files can be obtained from the Book's web site.
...
```{r,message=FALSE,warning=FALSE,fig.cap="Prediction vs. Truth"}
set.seed(100)
fit <- train(Class ~ ., data = training, method = "pls")
pred <- predict(fit, newdata = testing, type = "prob")
par(mar = c(5, 4, 2, 2) + 0.1)
plot(pred[, 2], jitter(unclass(testing$Class)-1, .1), ylim = c(-0.25, 1.25), yaxt = "n", ylab = "Credit Quality", xlab = "Predicted Probability of \"Good\"")
axis(2, 0:1, c("Bad", "Good"))
```
...
```{r}
pred.class = predict(fit, newdata = testing)
confusionMatrix(data = pred.class, testing$Class, positive = "Good")
```
