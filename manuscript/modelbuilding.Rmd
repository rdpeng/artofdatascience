...
```{r}
d <- read.csv("data/leanpub-rprogramming-interested-readers-20150430.csv", stringsAsFactors = FALSE)
x0 <- d$Willing.to.pay
set.seed(1)
...
qplot(x, y, main = "Simulated Data") + 
        geom_smooth(method = "lm", se = FALSE, lwd = 2, formula = "y ~ x")
```
...
qplot(temp, ozone, data = ny) + 
        geom_smooth(method = "lm", lwd = 2, se = FALSE, formula = "y ~ x")
```
...
qplot(temp, ozone, data = ny) + 
        geom_smooth(method = "loess", lwd = 2, se = FALSE, formula = "y ~ x")
```
